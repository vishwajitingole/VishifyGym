...
export function demoDashboard() {
  const today = todayId();
  const weekly = Array.from({ length: 7 }, (_, index) => { const d = new Date(`${today}T12:00:00`); d.setDate(d.getDate() - 6 + index); return { date: d.toLocaleDateString('en-CA'), label: d.toLocaleDateString('en', { weekday: 'short' }), eggs: [3, 4, 5, 2, 5, 4, 2][index], dahiBowls: [1, 1, 2, 1, 1, 2, 1][index], protein: [29, 35, 52, 23, 41, 46, 23][index], workedOut: [true, true, false, true, true, true, false][index] }; });
  return { today: { date: today, eggs: 2, dahiBowls: 1, proteinTarget: 50, calorieTarget: 900, cardioTargetMinutes: 20, nutrition: { protein: 23, calories: 324 } }, weekly, workouts: [], forecast: { eggs: 25, dahiBowls: 9 }, streaks: { protein: 0, cardio: 0 }, coach: { pillars: { fuel: false, train: false, cardio: false }, counts: { push: 0, pull: 0, pushups: 0, cardio: 0, eggs: 0, dahiBowls: 0 }, focus: 'A clean slate. Start with a Push day.', trend: { dir: 'flat', pct: 0 }, tip: 'Log any pillar — the coach starts advising from real data.' }, user: null };
}