...
router.get('/last/:type', async (req, res, next) => {
  try { res.json(await WorkoutSession.findOne({ type: req.params.type }).sort({ date: -1, createdAt: -1 }) || null); } catch (error) { next(error); }
});
...
router.post('/', async (req, res, next) => {
  try {
    const session = { ...req.body, totalVolume: volumeFor(req.body.exerciseLogs) };
    ...
    if (names.length) {
      const previous = await WorkoutSession.find({ 'exerciseLogs.exerciseName': { $in: names } });
