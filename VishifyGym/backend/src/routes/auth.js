...
router.post('/register', async (req, res, next) => {
  try {
    ...
    res.status(201).json({ token: signToken(user), user: { _id: user._id, name: user.name, email: user.email, schedule: user.schedule || {} }, seeded: true });
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    await seedUserExercises(user._id);
    res.json({ token: signToken(user), user: { _id: user._id, name: user.name, email: user.email, schedule: user.schedule || {} } });
  } catch (error) { next(error); }
});
...
router.get('/me', requireAuth, async (req, res, next) => {
  try {
    ...
    res.json({ _id: user._id, name: user.name, email: user.email, proteinTarget: user.proteinTarget, calorieTarget: user.calorieTarget, cardioTargetMinutes: user.cardioTargetMinutes, schedule: user.schedule || {} });
  } catch (error) { next(error); }
});
...
router.patch('/me', requireAuth, async (req, res, next) => {
  try {
    ...
    const scheduleTypes = new Set(['push', 'pull', 'rest']);
    const sanitized = {};
    Object.entries(req.body || {}).forEach(([key, value]) => {
      if (key === 'schedule') {
        if (value && typeof value === 'object' && !Array.isArray(value)) {
          const next = {};
          Object.entries(value).forEach(([day, type]) => {
            const d = Number(day);
            if (Number.isInteger(d) && d >= 0 && d <= 6 && scheduleTypes.has(type)) next[String(d)] = type;
          });
          sanitized.schedule = next;
        }
        return;
      }
